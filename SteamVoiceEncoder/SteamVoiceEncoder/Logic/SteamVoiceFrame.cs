...
namespace SteamVoiceEncoder.Logic
{
    class SteamVoiceFrame
    {
        ...
        private uint crc32(byte[] buf, int maxLength)
        {
            uint c = 0xFFFFFFFF;
        ...
        public int EncodeSamplesToFrame(byte[] bufferAudioPCM, out byte[] nCompressed)
        {
            ...
            Buffer.BlockCopy(bufferHeader, 0, rv, 0, bufferHeader.Length);
            Buffer.BlockCopy(bufferOut, 0, rv, bufferHeader.Length, nCompressedBytes);

            uint crc = crc32(rv, dataSize);
